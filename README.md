...
A note on what that covers: the code is MIT. The KJV text itself is public
domain in the US (in the UK it sits under perpetual Crown copyright via letters
patent), and this repo does not redistribute it — `data/` is git-ignored, so
every clone fetches its own copy from bible-api.com.

Generated summaries are subject to the terms of whichever model produced them.
`qwen3-embedding` is Apache-2.0; Google's Gemma models ship under the Gemma
Terms of Use, which carry use restrictions. This does not affect the repo, since
summaries live only in the git-ignored `data/` — but check the terms before
publishing summaries generated by a restrictively licensed model.
